...
public class TwitterMapper
{
    ...
    {
        // call Twitter API to get MAX_TWEETS_PER_STATE tweets for the passed State object and keyword value
        double latitude = state.getCenter().getLatitude();
        double longitude = state.getCenter().getLongitude();
        ...
        int totalTweets = 0;
        double totalStateSentiment = 0.0;
        double tweetSentiment = 0.0;
        double aveStateSentiment = 0.0;

        for (Status status : result.getTweets())
        {
            totalTweets++;
            Tweet tweet = new Tweet(status.getUser().getName(), status.getText(), status.getCreatedAt(), status.getGeoLocation());

            totalStateSentiment += tweet.getSentiment();
        }

        if (totalTweets == 0) {
            aveStateSentiment = 0.0;
        } else {
